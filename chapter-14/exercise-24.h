...
/*
 * The Book class does not need custom copy or move assignment operators.
 * 1. Default Behaviour Sufficiency:
 *   The book class contains only:
 *     - std::string members (handle their own copy/move semantics)
 *     - int members (trivially copyable)
 *   The compiler-generate copy/move assignment operators are:
 *     - Correct: Member-wise assignment
 *     - Efficient: String move assignment is O(1)
 *     - Exception-safe: String operations provide strong guarantee
 * 2. Domain Model Considerations:
 *   Book objects represent immutable metadata:
 *     - Title, author, ISBN are fundamental identifiers
 *     - Publication year is historical fact
 *     - Genre classification is categorical
 *     - Page count is physical property
 *     - Copy count is inventory state (managed separately)
 *   Copy assignment semantics are straightforward:
 *     - Complete replacement of all metadata
 *     - No special resource management needed
 */

#ifndef CHAPTER_14_EXERCISE_24_H_
#define CHAPTER_14_EXERCISE_24_H_
...
#endif  // CHAPTER_14_EXERCISE_24_H_