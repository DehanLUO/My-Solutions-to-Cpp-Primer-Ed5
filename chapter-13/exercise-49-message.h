...
#ifndef CHAPTER_13_EXERCISE_49_MESSAGE_H_
#define CHAPTER_13_EXERCISE_49_MESSAGE_H_
...
class Message {
 ...
 public:
   ...
   * @param other Source Message to move from
   * @post Transfers ownership of contents and folder relationships
   ...
   * @param rhs Source Message to move from
   * @return Reference to this
   * @post Transfers ownership of contents and folder relationships
   * @note noexcept qualified for compatibility with STL containers
   * Time Complexity O(n + m) where n,m are folder counts
   * Space Complexity O(1)
   */
...
inline Message::Message(Message &&other) noexcept
    : contents_(std::move(other.contents_)),
      folders_(std::move(other.folders_)) {
  for (auto *folder : folders_) {
    folder->RemoveMessage(&other);
    folder->AddMessage(this);
  }
  other.folders_.clear();
}
...
#endif  // CHAPTER_13_EXERCISE_49_MESSAGE_H_